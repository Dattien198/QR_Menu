...
@section('header')
    <div>
        <h2 class="text-2xl font-bold text-slate-900">Add Menu Item</h2>
        <p class="text-sm text-slate-500">Create a new dish for your menu.</p>
    </div>
@endsection

@section('content')
<div class="max-w-4xl">
    <form action="{{ route('admin.menu-items.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        @csrf
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Item Name</label>
                    <input type="text" name="name" class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none transition-all" placeholder="e.g. Signature Beef Burger" required>
                </div>
                ...
                <div class="md:col-span-1">
                    ...
                    <input type="number" name="price" value="0" class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none transition-all" required>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Description</label>
                    <textarea name="description" rows="3" class="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500 outline-none transition-all" placeholder="Tell customers about this dish..."></textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" name="is_available" value="1" id="is_available" class="w-4 h-4 text-orange-600 border-slate-300 rounded focus:ring-orange-500" checked>
